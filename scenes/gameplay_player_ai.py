...
class GameplayPlayerAIScene(Scene):
    def __init__(self, screen, context, switch_scene):
        ...
        self.player_one = True  # Human plays as White
    ...
    def pre_event(self):
        self.human_turn = (self.game_state.white_to_move and self.player_one) or \
                ...
                if len(self.player_clicks) == 2:
                    ...
                    for i in range(len(self.valid_moves)):
                        if move == self.valid_moves[i]:
                            self.game_state.make_move(self.valid_moves[i], True)
                            self.move_made = True
                            self.animate = True
                            self.square_selected = ()
                            self.player_clicks = []
    ...
    def update(self):
        if not self.game_over and not self.human_turn and self.context.player_ai_func:
            ai_move = self.context.player_ai_func(self.game_state, self.valid_moves, "black")
            if ai_move is None:
